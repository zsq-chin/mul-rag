...
| 管理普通用户 | 允许 | 允许 | 禁止 |
| 管理管理员和角色 | 允许 | 禁止 | 禁止 |
| 在问答中选择或使用普通、多模态知识库 | 允许 | 允许 | 允许 |
| 在问答中使用知识图谱 | 允许 | 禁止 | 禁止 |
| 知识库、多模态、图谱、数据库、统计和系统管理页面 | 允许 | 禁止 | 禁止 |

### 10.2 后端强制鉴权

- 普通知识库列表、多模态知识库列表及检索图片代理允许三种已登录角色使用；所有知识库、多模态管理、图谱、数据库、统计和系统配置接口统一要求 `get_superadmin_user`。
- 聊天接口允许三种已登录角色使用普通和多模态知识库检索；当非超级管理员提交图谱选项时，后端明确拒绝，不能只依赖前端隐藏。
...
- 角色权限：三角色逐接口矩阵、管理员列表过滤、越权修改拒绝、普通用户可用知识库与多模态检索、非超级管理员伪造图谱参数拒绝。
