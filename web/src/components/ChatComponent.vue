  ...
  <div class="chat" :class="{ 'refs-sidebar-open': refsSidebarVisible && refsSidebarPinned }">
    ...
    <div class="chat-box" ref="messagesContainer" :class="{ 'wide-screen': meta.wideScreen, 'font-smaller': meta.fontSize === 'smaller', 'font-larger': meta.fontSize === 'larger' }">
      <MessageComponent
        ...
        @select-question="handleQuestionClick"
      >
      </MessageComponent>
    </div>
    <div class="bottom">
        ...
        >
          <template #options-left>
            ...
            <a-dropdown
              ...
              @openChange="handleMultimodalDropdownOpen"
            >
              <a class="ant-dropdown-link" @click.prevent="toggleMultimodalKb">
                <Layers style="margin-right: 3px;" size="14"/>
                <span class="text">{{ selectedMultimodalKb?.kbName || '多模态知识库' }}</span>
              </a>
              <template #overlay>
                <a-menu class="multimodal-kb-menu" :selectedKeys="meta.multimodal_kb_id ? [meta.multimodal_kb_id] : []">
                  <a-menu-item v-if="opts.multimodalKbsLoading" key="loading" disabled>
                    <span>加载中...</span>
                  </a-menu-item>
                  <a-menu-item v-else-if="opts.multimodalKbsError" key="error" disabled>
                    <span>{{ opts.multimodalKbsError }}</span>
                  </a-menu-item>
                  <a-menu-item v-else-if="opts.multimodalKbs.length === 0" key="empty" disabled>
                    <span>暂无多模态知识库</span>
                  </a-menu-item>
                  <template v-else>
                    <a-menu-item v-for="kb in opts.multimodalKbs" :key="kb.kbId" @click="useMultimodalKb(kb)">
                      <a href="javascript:;" class="multimodal-kb-option">
                        <span class="multimodal-kb-name">{{ kb.kbName }}</span>
                        <span v-if="kb.fileCount" class="multimodal-kb-meta">{{ kb.fileCount }} 文件</span>
                      </a>
                    </a-menu-item>
                  </template>
                  <a-menu-divider />
                  <a-menu-item key="off" @click="useMultimodalKb(null)">
                    <a href="javascript:;">不使用多模态知识库</a>
                  </a-menu-item>
                </a-menu>
              </template>
            </a-dropdown>
...
const opts = reactive({
  ...
  databases: [],
  multimodalKbs: [],
  multimodalKbsLoading: false,
  multimodalKbsLoaded: false,
  multimodalKbsError: '',
...
const loadMultimodalKbs = async () => {
  if (opts.multimodalKbsLoading || opts.multimodalKbsLoaded) return

  opts.multimodalKbsLoading = true
  opts.multimodalKbsError = ''
  try {
    const data = await chatApi.getMultimodalKbs()
    opts.multimodalKbs = Array.isArray(data?.kbs) ? data.kbs : []
    opts.multimodalKbsLoaded = true
  } catch (error) {
    console.error('加载多模态知识库列表失败', error)
    opts.multimodalKbsError = error.message || '加载失败'
  } finally {
    opts.multimodalKbsLoading = false
  }
}

const handleMultimodalDropdownOpen = (open) => {
  if (open) {
    loadMultimodalKbs()
  }
}

const toggleMultimodalKb = () => {
  meta.use_multimodal_kb = !meta.use_multimodal_kb
  if (meta.use_multimodal_kb) {
    loadMultimodalKbs()
  }
}

const useMultimodalKb = (kb) => {
  if (!kb) {
    meta.use_multimodal_kb = false
    meta.multimodal_kb_id = null
    meta.multimodal_kb_name = null
    return
  }

  meta.use_multimodal_kb = true
  meta.multimodal_kb_id = kb.kbId
  meta.multimodal_kb_name = kb.kbName
}
...
const groupRefs = (id) => {
  ...
  if (msg?.refs?.knowledge_base?.results?.length > 0) {
        ...
        for (let i = 0; i < lines.length - 1; i++) {
          ...
          if (line) {
            try {
              ...
              // 如果后端返回了 related_questions，将其添加到更新载荷中
              if (data.related_questions && Array.isArray(data.related_questions)) {
                updatePayload.related_questions = data.related_questions;
              }
...
const handleQuestionClick = (question) => {
  if (isStreaming.value) return; // 如果正在生成中，忽略点击
  conv.value.inputText = question;
  sendMessage();
};
</script>
...
.multimodal-kb-dropdown {
  ...
  .multimodal-kb-menu::-webkit-scrollbar {
    width: 6px;
  }

  .multimodal-kb-menu::-webkit-scrollbar-track {
    background: transparent;
  }

  .multimodal-kb-menu::-webkit-scrollbar-thumb {
    background: var(--gray-300);
    border-radius: 999px;
  }

  .multimodal-kb-menu::-webkit-scrollbar-thumb:hover {
