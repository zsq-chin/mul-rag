        ...
        for j in range(len(table.columns)):
            ...
            if row_cells[i][j] not in row_temp and col_cells[j][i] not in col_temp:  # 行列地址值去重
                if col_counts == 1 and row_counts == 1:  # 单元格没有合并
                    table_text = table_text + ' ' + table.rows[i].cells[j].text.replace("\n", "") + ' '
                elif col_counts != 1 and row_counts == 1:  # 横向合并
                    table_text = table_text + ' '. \
                        format(col_counts) + table.rows[i].cells[j].text + ' '
                elif col_counts == 1 and row_counts != 1:  # 纵向合并
                    table_text = table_text + ' '. \
                        format(row_counts) + table.rows[i].cells[j].text + ' '
                else:  # 横纵同时合并
                    table_text = table_text + ' '. \
                        format(col_counts, row_counts) + table.rows[i].cells[j].text + ' '
